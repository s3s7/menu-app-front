...
import { DbMenuData, getMenusAPI } from '@/api/menuApi'
import {
  SITE_META,
  GET_USER_ERROR_MSG,
...
export const useFirebaseAuth = () => {
  ...
  const [dbMenusData, setDbMenusData] = useState<DbMenuData[] | null>(null)
  ...
  const loginWithEmailAndPassword = async (email: string, password: string) => {
    ...
    try {
      ...
      if (isNotVerified) {
        ...
        const dbMenusData = await getMenusAPI(idToken, user.uid)
        setDbMenusData(dbMenusData)
