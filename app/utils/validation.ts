...
export const validateMenuTitle = (menuTitle: string) => {
  if (!/\S/.test(menuTitle)) {
    return '旅行タイトルを入力してください'
  }

  if (menuTitle.length > MAX_TRIP_TITLE_LENGTH) {
...
export const validateStartDate = (startDate: string) => {
  ...
  if (
    startDateObj < MIN_DATE_OBJ ||
    startDateObj > MAX_DATE_OBJ ||
...
export const validateEndDate = (startDate: string, endDate: string) => {
  ...
  if (
    endDateObj < MIN_DATE_OBJ ||
    endDateObj > MAX_DATE_OBJ ||
...
export const validateMenuMemo = (menuMemo: string) => {
  if (menuMemo.length > MAX_TRIP_MEMO) {
