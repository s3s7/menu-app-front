...
export const InputField: FC<InputFieldProps> = ({
  ...
  return (
    <div className="relative">
      <label
        className={`text-sm sm:text-base text-gray-500 whitespace-nowrap ${srOnly ? 'sr-only' : ''
          }`}
        htmlFor={id}
      >
        {labelName}
      ...
      <div className="relative flex items-center">
        <input
          ref={inputRef}
          className={`text-sm sm:text-base h-10 px-3 py-2 text-gray-700 border rounded focus:outline-none w-full ${error ? 'border-red-500' : 'focus:border-brand-color'
            } ${onCopy ? 'pr-8' : ''}`}
          type={type}
          min={min}
          max={max}
